...
const TransanctionNotificationModal: React.FC<ChildProps> = ({
  notification,
  modalclose,
  modalopen,
}) => {
      ...
      <Dialog.Content>
        ...
        >
          <Dialog.Title className="text-xl font-bold flex items-center justify-center">
            <Image src={logo} alt="logo" height={30} width={130} />
          </Dialog.Title>
          <div className="mt-2 grid  lg:w-full gap-[2rem]">
            <div className="bg- pl-[1rem] lg:pl-0 pr-[1rem] lg:pr-0 bg-[#F7F7F7] py-[0.5rem]  lg:w-full  flex  flex-row justify-between items-center  rounded-2xl ">
              <div className="grid gap-[0.3rem] pt-[0.3rem] lg:px-[1rem]">
                <div
                  className={`flex items-center justify-center font-[500] text-[15px] ${
                    notification.status === "completed"
                      ? "text-green-500 bg-green-100 py-[0.1rem] px-[0.1rem] rounded-md"
                      : notification.status === "pending"
                      ? "text-yellow-500 bg-yellow-100 py-[0.1rem] px-[0.1rem] rounded-md"
                      : "text-red-500 bg-red-100 py-[0.1rem] px-[0.1rem] rounded-md"
                  }`}
                >
                  {notification.status}
                </div>
                <div className="flex gap-[0.5rem] items-center justify-center">
                  <Image src={usd} height={20} width={20} alt="usd logo" />
                  <h3 className="font-[600] text-[18px] lg:text-[20px] text-black">
                    +{""}
                    {notification.crypto_value}
                  </h3>
                </div>
              </div>
              <h4 className="text-[14px] text-[#7C7C7C] font-[500] pr-[0.5rem]">
                You recieved {""}
            ...
            <div className="grid gap-[1rem] ">
              <div className="flex items-center justify-between">
                <p className="font-[500] text-[#7C7C7C] text-[16px]">
                  Crypto value
                </p>
                <div className="flex gap-[0.5rem] items-center justify-center">
                  <Image src={usd} height={17} width={17} alt="usd logo" />
                  <h3 className="font-[600] text-[16px] text-black">
                    +{""}
                    {notification.crypto_value}
                  </h3>
                </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`flex items-center justify-center font-[500] text-[13px] ${
                    notification.status === "completed"
                      ? "text-green-500 bg-green-100 py-[0.2rem] px-[0.3rem] rounded-md"
                      : notification.status === "pending"
                      ? "text-yellow-500 bg-yellow-100 py-[0.2rem] px-[0.3rem] rounded-md"
                      : "text-red-500 bg-red-100 py-[0.2rem] px-[0.3rem] rounded-md"
                  }`}
                >
                  {notification.status}
                </div>
          ...
          <div className="flex flex-row w-full gap-[2rem] items-center justify-center">
            <SecondaryButton
              className="w-full cursor-pointer py-2 bg-red-500 text-white rounded-lg hover:bg-red-600"
              onClick={modalclose}
            >
              Cancel
            </SecondaryButton>
            ...
            <PrimaryButton
              shortcut=" "
              className="flex flex-row items-center justify-center cursor-pointer w-[50%]"
            >
