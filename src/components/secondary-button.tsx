...
export const SmallSecondaryButton = ({ text = "View transaction", onClick }: { text?: string; onClick?: React.MouseEventHandler<HTMLDivElement> }) => {
  const [isPressed, setIsPressed] = React.useState(false);
  ...
  return (
    ...
    >
      <div
        ...
        onClick={onClick}
      >
        <span className="text-xs font-medium leading-[14px] tracking-tight" style={{ color: '#8D8D8D', fontFamily: "'SF Pro Text', sans-serif" }}>
          {text}
        </span>
      </div>
    </div>
